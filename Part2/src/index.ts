...
const encrypt = async (
  ...
  const iv = mimc7.F.toObject(mimc7.multiHash(plaintext, BigInt(0)))
  const ciphertext : Ciphertext = {
    iv, 
    data: plaintext.map((e: bigint, i: number): bigint => { return e + mimc7.F.toObject(mimc7.hash(sharedKey, iv + BigInt(i))) })
  }
...
const decrypt = async (
  ...
  const plaintext : Plaintext = ciphertext.data.map(
    (e: bigint, i: number): bigint => {return e - mimc7.F.toObject(mimc7.hash(sharedKey, ciphertext.iv + BigInt(i)))}
  )
